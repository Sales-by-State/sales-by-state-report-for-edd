...
( function ( wp, config ) {
	...
	var API = '/sbsedd/v1/report';
	var DIAGNOSTICS = '/sbsedd/v1/diagnostics';
	var BACKFILL = '/sbsedd/v1/backfill';
	var DEFAULT_PER_PAGE = 25;
	var TEXTDOMAIN = 'sales-by-state-report-for-edd';
	...
	function ImportStatus( props ) {
		...
		if ( ! diag.table_exists ) {
			return el(
				'div',
				{ className: 'sbsedd-notice is-error' },
				el( 'p', { className: 'sbsedd-notice__title' }, __( 'The report table is missing.', TEXTDOMAIN ) ),
		...
		return el(
			'div',
			{ className: 'sbsedd-importing', role: 'status', 'aria-live': 'polite' },
			el(
				'div',
				{ className: 'sbsedd-importing__row' },
				el( 'span', { className: 'sbsedd-importing__label' }, __( 'Reading past orders…', TEXTDOMAIN ) ),
				el( 'span', { className: 'sbsedd-importing__count' }, pct + '%' )
			),
			el(
				'div',
				{ className: 'sbsedd-progress' },
				el( 'div', { className: 'sbsedd-progress__bar', style: { width: pct + '%' } } )
			),
			el(
				'p',
				{ className: 'sbsedd-importing__hint' },
	...
	function readStatuses( query ) {
		var raw = query && query.sbseddStatuses;
	...
	function StatusFilter( props ) {
		...
		return el(
			'div',
			{ className: 'sbsedd-filter' },
			el( 'span', { className: 'sbsedd-filter__label' }, __( 'Order status', TEXTDOMAIN ) ),
			el( Dropdown, {
				className: 'sbsedd-filter__dropdown',
				contentClassName: 'sbsedd-status-popover',
				...
				renderToggle: function ( toggleProps ) {
					return el(
						Button,
						{
							className: 'sbsedd-filter__toggle',
				...
				renderContent: function () {
					return el(
						'div',
						{ className: 'sbsedd-status-popover__list' },
	...
	function Filter( props ) {
		return el(
			'label',
			{ className: 'sbsedd-filter' },
			el( 'span', { className: 'sbsedd-filter__label' }, props.label ),
			el(
				'select',
				{
					className: 'sbsedd-filter__select',
					value: props.value,
					onChange: function ( event ) {
	...
	function SalesByStateReport( props ) {
		...
		var country = query.sbseddCountry || settings.defaultCountry || 'US';
		var year = query.sbseddYear || settings.defaultYear;
		...
		var children = [
			...
			el(
				'div',
				{ key: 'filters', className: 'sbsedd-filters' },
				el( Filter, {
					label: __( 'Country', TEXTDOMAIN ),
					value: country,
					options: settings.countries || [],
					onChange: function ( value ) {
						setFilter( { sbseddCountry: value } );
					}
				} ),
				el( Filter, {
					...
					onChange: function ( value ) {
						setFilter( { sbseddYear: value } );
					}
				} ),
				el( StatusFilter, {
					value: statuses,
					onChange: function ( next ) {
						setFilter( { sbseddStatuses: next.join( ',' ) } );
					}
				} )
			)
		];
		...
		if ( data.error ) {
			children.push(
				el(
					'div',
					{ key: 'error', className: 'notice notice-error sbsedd-error' },
		...
		return el( 'div', { className: 'woocommerce-analytics__report sbsedd-report' }, children );
	}
	...
	function readUrlQuery() {
		var query = {};
		var keys = [ 'sbseddCountry', 'sbseddYear', 'sbseddStatuses', 'orderby', 'order', 'paged', 'per_page' ];
		...
		query.page = found.page || 'sbsedd-sales-by-state';
	...
	function mountStandalone() {
		function render() {
			var node = document.getElementById( 'sbsedd-root' );

			if ( ! node ) {
				return;
			}

			if ( node.getAttribute( 'data-sbsedd-mounted' ) ) {
				return;
			}

			node.setAttribute( 'data-sbsedd-mounted', '1' );
